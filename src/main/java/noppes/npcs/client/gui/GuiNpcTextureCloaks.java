...
public class GuiNpcTextureCloaks extends GuiNpcSelectionInterface {
   ...
   public GuiNpcTextureCloaks(EntityNPCInterface npc, GuiScreen parent) {
      ...
      super.title = "Select Cloak";
   }

   public void initGui() {
      super.initGui();
      int index = super.npc.display.cloakTexture.lastIndexOf("/");
      if(index > 0) {
         String asset = super.npc.display.cloakTexture.substring(index + 1);
         if(super.npc.display.cloakTexture.equals(super.assets.getAsset(asset))) {
   ...
   public void drawScreen(int i, int j, float f) {
      ...
      super.npc.renderYawOffset = (float)Math.atan((double)(f5 / 40.0F)) * 20.0F + 180.0F;
      super.npc.rotationYaw = (float)Math.atan((double)(f5 / 40.0F)) * 40.0F + 180.0F;
   ...
   public void elementClicked() {
      if(super.dataTextures.contains(super.slot.selected) && super.slot.selected != null) {
         super.npc.display.cloakTexture = super.assets.getAsset(super.slot.selected);
