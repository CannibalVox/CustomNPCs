...
public class GuiNpcTextureOverlays extends GuiNpcSelectionInterface {
   ...
   public GuiNpcTextureOverlays(EntityNPCInterface npc, GuiScreen parent) {
      ...
      super.title = "Select Overlay";
      super.parent = parent;
   }

   public void initGui() {
      super.initGui();
      int index = super.npc.display.glowTexture.lastIndexOf("/");
      if(index > 0) {
         String asset = super.npc.display.glowTexture.substring(index + 1);
         if(super.npc.display.glowTexture.equals(super.assets.getAsset(asset))) {
   ...
   public void elementClicked() {
      if(super.dataTextures.contains(super.slot.selected) && super.slot.selected != null) {
         super.npc.display.glowTexture = super.assets.getAsset(super.slot.selected);
